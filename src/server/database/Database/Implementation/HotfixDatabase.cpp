...
void HotfixDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(HOTFIX_SEL_DATA_TAG_X_HOUSE_DECOR_RECORD,  
       "SELECT ID, DataTagID, HouseDecorID FROM data_tag_x_house_decor_record WHERE (`VerifiedBuild` > 0) = ?", CONNECTION_SYNCH);  
    PREPARE_MAX_ID_STMT(HOTFIX_SEL_DATA_TAG_X_HOUSE_DECOR_RECORD,  
       "SELECT MAX(ID) + 1 FROM data_tag_x_house_decor_record", CONNECTION_SYNCH);  
  
    // House.db2  
    PrepareStatement(HOTFIX_SEL_HOUSE,  
       "SELECT ID, InternalName, HouseTypeID, MapID, Flags FROM house WHERE (`VerifiedBuild` > 0) = ?", CONNECTION_SYNCH);  
    PREPARE_MAX_ID_STMT(HOTFIX_SEL_HOUSE,  
       "SELECT MAX(ID) + 1 FROM house", CONNECTION_SYNCH);

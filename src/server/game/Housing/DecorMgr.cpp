...
#include "Log.h"  
#include "HousingEntities.h"   // ADDED  
...
{  
    ...
    {  
        Field* fields = result->Fetch();  
        uint32 placedDecorId    = fields[0].GetUInt32();       // ADDED  
        uint32 houseId          = fields[1].GetUInt32();       // ADDED  
        uint32 roomId           = fields[2].GetUInt32();       // ADDED  
        uint32 decorTemplateId  = fields[3].GetUInt32();       // ADDED  
        float positionX         = fields[4].GetFloat();        // ADDED  
        float positionY         = fields[5].GetFloat();        // ADDED  
        float positionZ         = fields[6].GetFloat();        // ADDED  
        float orientationX      = fields[7].GetFloat();        // ADDED  
        float orientationY      = fields[8].GetFloat();        // ADDED  
        float orientationZ      = fields[9].GetFloat();        // ADDED  
        float scale             = fields[10].GetFloat();       // ADDED  
        bool isExterior         = fields[11].GetBool();        // ADDED  
        bool isLocked           = fields[12].GetBool();        // ADDED  
        uint32 placementCost    = fields[13].GetUInt32();      // ADDED  
  
        auto decor = std::make_unique<PlacedDecor>();          // ADDED  
        decor->SetId(placedDecorId);                           // ADDED  
        decor->SetHouseId(houseId);                            // ADDED  
        decor->SetRoomId(roomId);                              // ADDED  
        decor->SetDecorTemplateId(decorTemplateId);            // ADDED  
        decor->SetPosition(positionX, positionY, positionZ);   // ADDED  
        decor->SetOrientation(orientationX, orientationY, orientationZ); // ADDED  
        decor->SetScale(scale);                                // ADDED  
        decor->SetExterior(isExterior);                        // ADDED  
        decor->SetLocked(isLocked);                            // ADDED  
        decor->SetPlacementCost(placementCost);                // ADDED  
  
        _placedDecor[placedDecorId] = std::move(decor);        // ADDED  
        ++count;  
    } while (result->NextRow());  

...
#ifndef TRINITYCORE_HOUSE_MGR_H
#define TRINITYCORE_HOUSE_MGR_H
...
public:
    ...
    House* GetHouse(uint32 houseId) const;
    bool CreateHouse(ObjectGuid playerGuid, uint32 plotId, uint32 houseTemplateId);
    bool RelinquishHouse(ObjectGuid playerGuid, uint32 houseId);
    bool MoveHouse(uint32 houseId, uint32 newPlotId);
...
private:
    ...
    std::map<ObjectGuid, std::vector<uint32>> _playerHouseMap;
};

#endif // TRINITYCORE_HOUSE_MGR_H
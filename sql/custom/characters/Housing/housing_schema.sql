-- ============================================================================  
-- Housing System Schema - Character Database Tables (NON-DESTRUCTIVE)  
-- ============================================================================  
--  
-- Creates all tables required by the WoW Housing system in the character  
-- database. Players own and customize houses within neighborhoods, place  
-- decorations, configure rooms and fixtures, and join community initiatives.  
--  
-- Tables:  
...
--                                  3=Exploration  
--  
-- NON-DESTRUCTIVE: Uses CREATE TABLE IF NOT EXISTS (never drops existing data).  
-- All new columns are baked into the CREATE TABLE definitions below.  
...
CREATE TABLE IF NOT EXISTS `character_housing_decor` (  
    ...
    `scale` FLOAT NOT NULL DEFAULT 1.0 COMMENT 'Uniform scale multiplier',  
...
CREATE TABLE IF NOT EXISTS `character_housing_rooms` (  
    ...
    `gridX` INT NOT NULL DEFAULT 0 COMMENT 'Grid X position',  
    `gridY` INT NOT NULL DEFAULT 0 COMMENT 'Grid Y position',  
    `floorIndex` INT NOT NULL DEFAULT 0 COMMENT 'Floor index',  
    ...
    `wallThemeId` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Per-surface wall theme',  
    `floorThemeId` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Per-surface floor theme',  
    `ceilingThemeId` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Per-surface ceiling theme',  
    PRIMARY KEY (`ownerGuid`, `id`)  
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;  
...
-- ---------------------------------------------------------------------------  
-- 12. neighborhood_initiative_task_progress - Per-task progress  
--  
-- Tracks progress toward each task within a neighborhood initiative.  
-- ---------------------------------------------------------------------------  
...
-- ---------------------------------------------------------------------------  
-- account_housing_blueprint - Saved house blueprints (account-wide)  
--  
-- Serialized house layouts a player can export and re-import. Column types  
-- mirror the bind calls in HousingBlueprintMgr.cpp (setUInt64/setString/  
-- setUInt32/setUInt8/setInt64/setBinary). id is server-assigned (no AUTO_INCREMENT).  
-- ---------------------------------------------------------------------------  
CREATE TABLE IF NOT EXISTS `account_housing_blueprint` (  
    `id` BIGINT UNSIGNED NOT NULL COMMENT 'Blueprint ID (server-assigned)',  
    `uuid` VARCHAR(36) NOT NULL DEFAULT '' COMMENT '36-char UUID string',  
    `bnetAccountId` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Battle.net account ID',  
    `exporterGuid` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Player GUID who exported',  
    `name` VARCHAR(64) NOT NULL DEFAULT '' COMMENT 'Blueprint display name',  
    `type` TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Blueprint type',  
    `flags` TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Blueprint flags',  
    `createTime` BIGINT NOT NULL DEFAULT 0 COMMENT 'Unix timestamp of creation',  
    `content` LONGBLOB NULL COMMENT 'Serialized blueprint bytes',  
    PRIMARY KEY (`id`),  
    UNIQUE INDEX `idx_uuid` (`uuid`),  
    INDEX `idx_bnet` (`bnetAccountId`)  
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;